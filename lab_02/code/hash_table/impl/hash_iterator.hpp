...
HashTableIterator<Key, Value>::HashTableIterator(HashTable<Key, Value> &hash_table)
...
//TODO
template<typename Key, typename Value>
HashTableIterator<Key, Value>::HashTableIterator(HashTable<Key, Value> &hash_table, size_t bucket_idx, NodeIterator node_it)
    : table_ptr(&hash_table),
...
template<typename Key, typename Value>
HashTableIterator<Key, Value>::HashTableIterator(const HashTableIterator<Key, Value> &other)
...
//TODO
template<typename Key, typename Value>
std::pair<Key, Value> &HashTableIterator<Key, Value>::operator*() {
    checkNotEnd();
    checkValid();
    return *current_node;
}
...
template<typename Key, typename Value>
const std::pair<Key, Value> &HashTableIterator<Key, Value>::operator*() const {
    checkNotEnd();
    checkValid();
    return *current_node;
}
...
template<typename Key, typename Value>
HashTableIterator<Key, Value>::operator bool() const {
    return table_ptr &&
           current_bucket < total_buckets &&
...
template<typename Key, typename Value>
Key &HashTableIterator<Key, Value>::getKey() {
    checkNotEnd();
    checkValid();
...
template<typename Key, typename Value>
const Key &HashTableIterator<Key, Value>::getKey() const {
    checkNotEnd();
    checkValid();
...
template<typename Key, typename Value>
Value &HashTableIterator<Key, Value>::getValue() {
    checkNotEnd();
    checkValid();
...
template<typename Key, typename Value>
const Value &HashTableIterator<Key, Value>::getValue() const {
    checkNotEnd();
    checkValid();
    return current_node->second;
}
...
template<typename Key, typename Value>
HashTableIterator<Key, Value> &HashTableIterator<Key, Value>::operator=(const HashTableIterator<Key, Value> &other) {
    if (this != &other) {
        table_ptr = other.table_ptr;
        current_bucket = other.current_bucket;
        current_node = other.current_node;
...
template<typename Key, typename Value>
HashTableIterator<Key, Value> HashTableIterator<Key, Value>::operator++(int) {
    HashTableIterator<Key, Value> tmp(*this);
    ++(*this);
    return tmp;
}

template<typename Key, typename Value>
HashTableIterator<Key, Value> &HashTableIterator<Key, Value>::operator+=(int value) {
    if (value < 0) {
        time_t currentTime = time(NULL);
...
template<typename Key, typename Value>
HashTableIterator<Key, Value> HashTableIterator<Key, Value>::operator+(int value) const {
    HashTableIterator<Key, Value> tmp(*this);
    tmp += value;
    return tmp;
}
...
//TODO
template<typename Key, typename Value>
bool HashTableIterator<Key, Value>::operator==(const HashTableIterator<Key, Value> &other) const {
...
template<typename Key, typename Value>
bool HashTableIterator<Key, Value>::isEnd() const {
    return current_bucket >= total_buckets || total_buckets == 0;
}

//TODO
template<typename Key, typename Value>
bool HashTableIterator<Key, Value>::isValid() const {
    if (!table_ptr || isEnd())
        return false;
...
//TODO
template<typename Key, typename Value>
void HashTableIterator<Key, Value>::moveToLastElement() {
...
//TODO
template<typename Key, typename Value>
void HashTableIterator<Key, Value>::moveToPreviousBucket() {
...
template<typename Key, typename Value>
void HashTableIterator<Key, Value>::checkNotEnd() const {
    if (current_bucket >= total_buckets) {
        time_t currentTime = time(NULL);
        throw IndexError(__FILE__, typeid(*this).name(), __LINE__, ctime(&currentTime));
    }
...
template<typename Key, typename Value>
void HashTableIterator<Key, Value>::checkValid() const {
    if (current_node == table_ptr->buckets[current_bucket].end()) {
        time_t currentTime = time(NULL);
