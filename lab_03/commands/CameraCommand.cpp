...
AddCamera::AddCamera(const ID &id, const Vertex &location) : _id(id), _location(location) {}
...
void DeleteCamera::execute() {
    Scene::iterator objIt = _scene->getObjectIter(_id);
    _scene->removeObject(objIt);
}


MoveCamera::MoveCamera(const double dx, const double dy, const double dz, const size_t id) : _dx(dx), _dy(dy), _dz(dz), _id(id) {}
